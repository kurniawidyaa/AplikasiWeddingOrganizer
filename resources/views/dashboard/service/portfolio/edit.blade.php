<x-admin-app-layout title="Edit Portfolio">
    <div class="col-md-8">
        <div class="card card-secondary">
            ...
            {{-- owner side --}}
            @auth('owner')
            <form action="{{ route('owner.port.update', $portfolio->id) }}" method="POST" enctype="multipart/form-data">
                @method('PUT')
                @csrf
            @endauth
            ...
            {{-- admin side --}}
            @auth('admin')
            <form action="{{ route('admin.port.update', $portfolio->id) }}" method="POST" enctype="multipart/form-data">
                @method('PUT')
                @csrf
            @endauth
                <div class="card-body"> 
